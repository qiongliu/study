...
/* ******************* */
/* 2012-4-19 ie9.0 Version*/
/* ******************* */
SWFUpload.getIEversion9 = function () {
    var isIE = !!window.ActiveXObject;
    var ua = navigator.userAgent.toLowerCase();
    if (isIE) {
        ieVision = ua.match(/msie ([\d.]+)/)[1];
        if (parseInt(ieVision.split(".")[0]) >= 9) {
            return true;
        } else {
            return false;
        }
    ...
    try {
        if (this.movieElement && typeof (movieElement.CallFunction) === "unknown") { // We only want to do this in IE
            ...
            for (var key in movieElement) {
                try {
                    if (typeof (movieElement[key]) === "function" && key[0] <= 'Z') { // 2012-4-19,Edit add " && key[0] <= 'Z' [ie9]"
